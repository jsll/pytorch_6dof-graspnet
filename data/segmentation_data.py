...
from util.util import is_mesh_file, pad, create_sseg_files, is_empty
import numpy as np
...
class SegmentationData(BaseDataset):
    def __init__(self, opt):
        ...
        self.seg_paths = self.get_seg_files(self.paths,
                                            os.path.join(self.root, 'seg'),
                                            seg_ext='.eseg')
        if opt.grasping:
            self.grasp_paths = self.get_seg_files(self.paths,
                                                  os.path.join(
                                                      self.root, 'grasps'),
                                                  seg_ext='.npy')
        if is_empty(os.path.join(self.root, 'sseg')):
            create_sseg_files(os.path.join(self.root, 'seg'),
                              os.path.join(self.root, 'sseg'))
    ...
    def __getitem__(self, index):
        ...
        meta['label'] = label
        if self.grasping:
            grasp = read_grasp(self.grasp_paths[index]) - self.offset
            grasp = pad(grasp, self.opt.ninput_edges, val=0, dim=0)
            meta['grasp_depth'] = 25 * grasp[:, 0]
            meta['grasp_sin_angle'] = grasp[:, 1]
            meta['grasp_cos_angle'] = grasp[:, 2]
    ...
    def get_seg_files(paths, seg_dir, seg_ext='.seg'):
        ...
        for path in paths:
            segfile = os.path.join(
                seg_dir,
                os.path.splitext(os.path.basename(path))[0] + seg_ext)
