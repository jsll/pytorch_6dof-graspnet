...
def get_files(root_dir,
    ...
    for split_file in split_files:
        ...
        if should_go_through:
            files += [
                os.path.join(root_dir, grasps_folder_name, f)
                for f in json.load(
                    open(os.path.join(root_dir, splits_folder_name,
                                      split_file)))[training_splits]
            ]
    return files
...
def make_parser():
    parser = argparse.ArgumentParser(
        description='6 DOF GraspNet parser.',
        formatter_class=argparse.ArgumentDefaultsHelpFormatter)

    parser.add_argument('--num_objects_per_batch',
                        type=int,
                        default=1,
    ...
    parser.add_argument('--depth_noise', type=float,
    ...
    parser.add_argument('--train_evaluator',
                        type=int,
                        default=0,
    ...
    parser.add_argument(
        ...
        default='',
        help=
        'if set to non-empty string, it initializes from the given checkpoint path'
    )
    parser.add_argument(
        '--allowed_categories',
        type=str,
    ...
    parser.add_argument('--blacklisted_categories',
                        type=str,
                        default='',
                        help='The opposite of allowed categories')
    parser.add_argument('--seed',
                        help='Number of objects to render in scene',
                        type=int,
                        default=-1)
    parser.add_argument('--save_steps',
                        help='How often to save logs and metrics',
                        type=int,
                        default=50)
    ...
    parser.add_argument(
        ...
        default=1.0,
        help=
        'used for checkng the effect of number of grasps per object on the success of the model.'
    )
    parser.add_argument(
        '--model_scale',
        type=int,
...
def transform_control_points_numpy(gt_grasps, batch_size):
    ...
    for i in range(batch_size):
        control_points[i, :, :] = control_points[i, :, :].dot(
            gt_grasps[i, :, :].T)
