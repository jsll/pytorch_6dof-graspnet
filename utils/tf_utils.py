...
def transform_control_points(gt_grasps,
    ...
    if mode == 'qt':
        ...
        with tf.variable_scope(scope):
            ...
            gt_grasps = tf.tile(tf.expand_dims(input_gt_grasps, 1),
        ...
        with tf.variable_scope(scope):
            ...
            return tf.matmul(control_points,
                             gt_grasps,
                             transpose_a=False,
                             transpose_b=True)
...
def conj_quaternion(q):
    ...
    q_conj = tf.concat([q_conj[0], -q_conj[1], -q_conj[2], -q_conj[3]],
                       axis=-1)
    return q_conj
...
def tf_rotation_matrix(az, el, th, batched=False):
    if batched:
        ...
        rx = tf.concat([ones, zeros, zeros, zeros, cx, -sx, zeros, sx, cx],
                       axis=-1)
        ry = tf.concat([cy, zeros, sy, zeros, ones, zeros, -sy, zeros, cy],
                       axis=-1)
        rz = tf.concat([cz, -sz, zeros, sz, cz, zeros, zeros, zeros, ones],
                       axis=-1)
